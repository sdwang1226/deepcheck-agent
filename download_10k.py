...
from deepcheck.embeddings import BGEEmbeddings

BASE = config.BASE_DIR
DATA_DIR = config.FILINGS_DIR
INDEX_DIR = config.INDEX_DIR

# SEC EDGAR requires a User-Agent with contact info
HEADERS = {
    "User-Agent": os.getenv("SEC_USER_AGENT", "DeepCheck Research Agent research@example.com"),
        ...
        try:
            ...
            if text and len(text) > 5000:
                ...
                registry[ticker] = {
                    "name": name,
                    "chunks": n,
                    "index_path": os.path.relpath(f"{INDEX_DIR}/{ticker.replace('.', '_')}", BASE),
                }
                total_chunks += n
            else:
                print("   ⚠️ 文本过短或下载失败，跳过", flush=True)
